...
import com.example.p3l_i_10796.databinding.FragmentMemberProfilBinding
import com.example.p3l_i_10796.models.Auth
import com.example.p3l_i_10796.models.Member
...
class MemberProfilFragment : Fragment() {

    private var _binding: FragmentMemberProfilBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMemberProfilBinding.inflate(inflater, container, false)
                ...
                { response ->
                    ...
                    for(i in 0 until id) {
                        var data = Member(
                            ...
                            jo.getJSONArray("data").getJSONObject(i).getString("DEPOSIT_SISA")
                        )
                        member.add(data)
                    }
                    binding.nama.setText("Nama : "+jo.getJSONArray("data").getJSONObject(0).getString("NAMA_MEMBER"))
                    binding.Email.setText("Email : "+jo.getJSONArray("data").getJSONObject(0).getString("EMAIL_MEMBER"))
                    if(jo.getJSONArray("data").getJSONObject(0).getString("MASA_AKTIVASI") == "null"){
                        binding.masaBerlaku.setText("Belum Aktivasi")
                    }else{
                        binding.masaBerlaku.setText("Masa Aktivasi : "+jo.getJSONArray("data").getJSONObject(0).getString("MASA_AKTIVASI"))
                    }
                    binding.sisaDepositUang.setText("Sisa Deposit Uang : Rp."+jo.getJSONArray("data").getJSONObject(0).getString("SISA_DEPOSIT_UANG"))
                    if(jo.getJSONArray("data").getJSONObject(0).getString("DEPOSIT_SISA") == "null"){
                        binding.depositsisa.setText("Sisa Deposit Kelas : 0")
                    }else {
                        binding.depositsisa.setText("Sisa Deposit Kelas : " + jo.getJSONArray("data").getJSONObject(0).getString("DEPOSIT_SISA"))
                    }

                    FancyToast.makeText(requireActivity(),"Menampilkan profil member", FancyToast.LENGTH_SHORT, FancyToast.SUCCESS, false).show()
                },
